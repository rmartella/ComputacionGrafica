...
bool Texture::load() {
	...
	/*
	* Esto se realiza para convertir de BGRA a RGBA
	* Descomentar para que no invierta las texturas que cargan
	*/
	/*for (int j = 0; j < imageHeight; j++) {
		for (int i = 0; i < imageWidth * 4; i+=4) {
			GLubyte b = textureData[i + j * (imageWidth * 4)];
			GLubyte g = textureData[i + j * (imageWidth * 4) + 1];
			GLubyte r = textureData[i + j * (imageWidth * 4) + 2];
			textureData[i + j * (imageWidth * 4)] = r;
			textureData[i + j * (imageWidth * 4) + 1] = g;
			textureData[i + j * (imageWidth * 4) + 2] = b;
		}
	}*/
